...
pub(super) unsafe extern "C" fn begin_paint(context: *mut lib::rdp_context) -> lib::BOOL {
...
pub(super) unsafe extern "C" fn end_paint(context: *mut lib::rdp_context) -> lib::BOOL {
    let mut raw = NonNull::new(context as *mut RawRdpContext).unwrap();
    let context = unsafe { raw.as_mut() };
    let mut callbacks = context.callbacks_mut();
...
pub(super) unsafe extern "C" fn post_connect(instance: *mut lib::rdp_freerdp) -> lib::BOOL {
    ...
    let Some(mut update) = NonNull::new(context.common.update) else {
        eprint!("update: null");
        return 0;
    };
...
pub(super) unsafe extern "C" fn post_disconnect(instance: *mut lib::rdp_freerdp) {
    let mut raw = NonNull::new(instance).unwrap();
    let context = unsafe { raw.as_mut() }.context;
    let Some(mut context) = NonNull::new(context as *mut RawRdpContext) else {
        eprint!("context: null");
...
pub(super) unsafe extern "C" fn verify_x509_certificate(
